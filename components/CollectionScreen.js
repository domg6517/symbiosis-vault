...
export default function CollectionScreen({ ownedCards, onCardClick, onScan, onLeaderboard, onProfile, onViewCollector, session }) {
  ...
  const ownedUltraRares = linked.filter((c) => c.rarity === "ultra_rare");
  const ownedUltraCount = ownedUltraRares.length;
        ...
        {view === "ultra" && (
          <div style={{ padding: "14px 20px" }}>
            ...
            {SINGLES.map((song) => {
              ...
              const songOwnedCount = songURs.filter((ur) =>
                ownedUltraRares.some((c) => c.songId === ur.songId && c.perspective === ur.perspective)
              ).length;
              return (
                <div key={song.id} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {songURs.map((ur) => {
                      const ownedCard = ownedUltraRares.find((c) => c.songId === ur.songId && c.perspective === ur.perspective);
                      const pLabel = ur.perspective === "J&J" ? "J&J" : ur.perspective.split(" ")[1];
                      return (
                        <div key={ur.id}
                          onClick={ownedCard ? () => onCardClick(ownedCard) : undefined}
                          style={{
