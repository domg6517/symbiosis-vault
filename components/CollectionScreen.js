...
export default function CollectionScreen({ ownedCards, onCardClick, onScan, onLeaderboard, onProfile, onViewCollector, session }) {
  ...
  // Derive ultra rare ownership from ownedCards prop (populated from API)
  const ownedURCards = ownedCards.filter(c => c.rarity === "ultra_rare" && c.linked);
  const ULTRA_RARES = generateUltraRares().map(ur => {
    const match = ownedURCards.find(c => c.songId === ur.songId && c.perspective === ur.perspective);
    return { ...ur, owned: !!match, imageUrl: match?.imageUrl || null };
  });
                ...
                <div key={song.id} style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {songURs.map((ur) => {
                      ...
                      return (
                        <div key={ur.id} style={{
                          flex: 1, padding: ur.owned ? "6px 6px 8px" : "10px 6px", textAlign: "center",
                          ...
                          {ur.owned ? (
                            <>
                              {ur.imageUrl ? (
                                <img
                                  src={ur.imageUrl}
                                  style={{ width: "100%", aspectRatio: "3/4", objectFit: "cover", borderRadius: 4, marginBottom: 4, display: "block", position: "relative", zIndex: 1 }}
                                  alt={pLabel}
                                />
                              ) : (
                                <StarIcon size={12} />
                              )}
                              <div style={{ fontSize: 11, fontFamily: SERIF, fontWeight: 300, color: C.cream, marginTop: 4, position: "relative", zIndex: 1 }}>{pLabel}</div>
